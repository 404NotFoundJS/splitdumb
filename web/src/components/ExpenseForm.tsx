...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ onExpenseAdded, refresh }) => {
    ...
    const categories = ['Food', 'Transport', 'Entertainment', 'Accommodation', 'Shopping', 'Other'];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!description || amount <= 0 || !payer || participants.length === 0) return;
        ...
        <div className="card form-card">
            <div className="card-body">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        <label htmlFor="expensePayer" className="form-label">Who Paid?</label>
                        <select
                            id="expensePayer"
                        ...
                        <div className="participants-group">
                            {users.map((user) => (
                                <div className="form-check" key={user.id}>
                                    ...
                                    />
                                    <label className="form-check-label" htmlFor={`participant-${user.id}`}>
                                        {user.name}
                                    </label>
                                </div>
                            ))}
