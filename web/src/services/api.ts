...
export const createExpense = async (description: string, amount: number, payer: string, participants: string[], category?: string) => {
    const response = await axios.post(`${API_URL}/expenses`, { description, amount, payer, participants, category });
    return response.data;
};
